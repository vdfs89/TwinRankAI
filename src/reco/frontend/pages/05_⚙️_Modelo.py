...
with col1:
    ...
    st.write(
        ...
        "**In-Batch Negative Sampling**. Durante o treino, para um dado usuário, todos os outros itens do batch que "
        "não foram interagidos por ele são considerados amostras negativas. Isso otimiza o uso de memória GPU e acelera a convergência."
    )

with col2:
    st.markdown("### 3. Approximate Nearest Neighbors (FAISS)")
    st.write(
        "Em tempo de inferência (produção), calcular a similaridade de um usuário contra 1 milhão de itens via *Dot Product* exato seria inviável (latência alta). "
    ...
    st.write(
        ...
        "Requisições de usuários populares ou bots batem diretamente no cache, poupando processamento neural e aliviando a carga do backend e do índice vetorial."
    )

st.markdown("---")
st.info(
    "Essa combinação (Two-Tower + In-Batch Negatives + FAISS + Redis) é o padrão da indústria (usado em YouTube, Pinterest, etc.) "
