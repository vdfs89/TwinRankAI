...
with col2:
    ...
    st.markdown("<h3 style='color: #9CA3AF; margin-top: 0;'>Industrial Deep Learning Recommendation Engine</h3>", unsafe_allow_html=True)
...
with col1:
    st.metric(label="Users Processed", value="1.4M", delta="RetailRocket")
with col2:
    st.metric(label="Products Catalog", value="235K", delta="Live")
with col3:
    st.metric(label="Recall@10 (Two-Tower)", value="0.81", delta="+42% vs Baseline")
with col4:
    st.metric(label="Embeddings Dimension", value="128", delta="FAISS Index")
...
with col1:
    if st.button("🚀 Ir para o Live Demo", use_container_width=True):
        st.switch_page("pages/04_🚀_Recommendations.py")

with col2:
    st.link_button("📄 GitHub Repository", "https://github.com/vdfs89/TwinRankAI", use_container_width=True)
